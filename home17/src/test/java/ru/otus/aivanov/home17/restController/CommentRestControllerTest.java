...
import ru.otus.aivanov.home17.dto.CommentUpdateDto;
import ru.otus.aivanov.home17.mapper.CommentMapper;
import ru.otus.aivanov.home17.mapper.CommentMapperImpl;

import ru.otus.aivanov.home17.security.SecurityConfiguration;
import ru.otus.aivanov.home17.services.CommentService;
...
class CommentRestControllerTest {
    ...
    @Test
    @WithMockUser(username = "admin", authorities = {"ROLE_ADMIN"})
    void editSaveShouldNotCallModifyMethodOfCommentService() throws Exception {
        this.mvc.perform(put("/api/comments/1")
                .contentType(APPLICATION_JSON)
                .content(mapper.writeValueAsString(new CommentUpdateDto(1L, "комментарий")))
    ...
    void createSaveShouldNotCallCreateMethodOfCommentService() throws Exception {
        this.mvc.perform(post("/api/comments")
                .contentType(APPLICATION_JSON)
                .content(mapper.writeValueAsString(new CommentCreateDto(1L, "Комментарий")))
        ).andExpect(status().isForbidden());
    ...
    @Test
    @WithMockUser(username = "user", authorities = {"ROLE_USER"})
    void removeShouldNotCallRemoveMethodOfCommentService() throws Exception {
        this.mvc.perform(delete("/api/comments/1")
                )
                .andExpect(status().isForbidden());
